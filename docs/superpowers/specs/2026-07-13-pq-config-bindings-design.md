...
Ordering: dense `pq` after `set_quantization` (engine rejects PQ when V4 int8 quantize is enabled — mutually exclusive, so `set_pq_config` returns nonzero and PQ stays off — the intended, non-fatal outcome); token `multivectorPq` after `set_rerank_config` (token PQ requires rerank configured — else `set_multivector_pq_config` returns nonzero, stays off).

**Token-PQ ↔ int8 `.mvec` interaction (both bindings, discovered in impl):** the binding's pre-existing `rerank.quantize` default is INT8, which puts the `.mvec` token pool in int8 mode — mutually exclusive with token-PQ (they are two compressions of the same pool). So `multivectorPq` + a bare `rerank={dimension:N}` would silently no-op. Resolution: requesting `multivectorPq` is treated as intent for token-PQ, so the binding defaults the rerank tier to **float** (`option_rerank_quant = RERANK_QUANT_FLOAT`) when `multivectorPq` is requested AND the caller did not explicitly pin `rerank.quantize`. An explicit `rerank.quantize:"int8"` still wins → token-PQ stays off (non-fatal), and `build_multivector_pq()` reports it unconfigured — the documented conflict, consistent with the dense `quantize:"int8"` + `pq` case. Implemented via an `option_rerank_quant_explicit` flag set in the rerank parse and consulted just before `set_rerank_config`. `set_pq_resident_tier`/`set_multivector_resident_tier` only when the tier key was given (leaving the engine's FLOAT default otherwise). Config restored on reopen is handled by the engine (`load_pq_config`/`load_multivector_pq_config`), so re-passing the same bag is idempotent.
