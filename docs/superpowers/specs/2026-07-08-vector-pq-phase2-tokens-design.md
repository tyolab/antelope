...
token-level sibling of the Phase-1 dense `.pq` codec — so per-token vectors are PQ-compressed and served both
as an ADC-scored MaxSim and (via `ANT_vector_source`) as the V6 token-HNSW graph backend, with configurable
replace / rerank query postures.
...
`ANT_vector_source` so the same codec serves ADC-MaxSim, the token-HNSW backend, and an exact float-MaxSim
rerank shortlist — selected by the `set_multivector_pq_config` config.
...
Deferred: dropping the resident float pool for the full replace-mode memory win (issue #19 class, tokens),
OPQ, global codebooks, `k != 256`, Node/Python bindings.
...
- **Token API (mirrors `ANT_multivector_store`'s V6 accessors):** `token_count`, `token_has`,
  ...
  `max_vector_count`, `get_m`. Plus `ANT_multivector_pq_source : public ANT_vector_source` (node == token;
  `is_quantized()`→true, `get()`→NULL, `reconstruct`/`score` via ADC) so `ANT_token_index`/`ANT_hnsw` can build
  a graph over PQ codes with no new graph code.
...
- **Token-HNSW backend:** when multivector-PQ is configured, the V6 `.tann` graph builds over the
  `ANT_multivector_pq_source` (memory-light graph over PQ codes) instead of the float
  `ANT_multivector_source`; over float otherwise. The `ANT_hnsw` int8 branch already uses `reconstruct` (never
  `get`), so a source with `get()==NULL` under `is_quantized()==true` is safe (same seam as dense PQ).
- **Query posture (both, configurable — `set_multivector_pq_config(m, posture, rerank_quant)`, mirroring
  Phase 1's replace/rerank):**
  - `posture = replace` → `search_multivector` runs token-HNSW candidate generation (over PQ codes) → the
    final MaxSim scoring is **ADC-MaxSim** via `multivector_pq->maxsim` (approximate final order, the route to
    dropping the resident float later).
  - `posture = rerank` → candidate generation → **exact float MaxSim rescore** via the resident `multivectors`
    ...
    dropped, deferred).
  - `search_rerank`'s existing candidate → MaxSim → publish shape is reused; ANN affects WHICH docs are
    considered, the posture affects HOW they are finally scored.
- **Coexistence with the V6 int8 token pool:** a segment's token pool is float **or** `.mvec`-int8 **or**
  ...
  are **byte-identical** when multivector-PQ is unconfigured (all changes gated on the config / a non-NULL
  `.mvpq` store).
- **Borrowed-store lifetime (the V6 UAF class — MANDATORY):** `ANT_token_index` borrows (does not own) the
  store it is built over. When `.tann` is built over `multivector_pq`, the `token_index` now borrows
  `multivector_pq`; any path that deletes/reloads `multivector_pq` while a non-empty `token_index` still points
  at it is a use-after-free. Compaction must therefore refresh `multivector_pq` **before** rebuilding `.tann`
  (the exact ordering that fixed the V6 `.mvec`↔`.tann` UAF), and teardown must free `multivector_pq` (the
...
- **Compaction:** rebuild `.mvpq` over the merged token pool with a fresh per-segment codebook + renumbered
  ...
  in-memory store. **Respect the borrowed-store ordering:** refresh `multivector_pq` AFTER its `.mvpq` is
  finalized and BEFORE the `.tann` rebuild (so the token_index binds the final retained pointer).
...
- **Store (`ANT_multivector_pq_store` unit):** save/load round-trip; forgiving load (missing / truncated /
  ...
  across runs).
- **Integration:** `.mvpq` as a token-HNSW backend — `search_multivector` recall vs a float-backed graph on
  the same tokens ≥ threshold; **replace** posture returns sane approximate top-k; **rerank** posture recall ≥
  replace and ≥ 0.9 vs exact MaxSim; coexistence — an index configured `.mvec`-int8 vs `.mvpq`-PQ are mutually
  exclusive per segment and both correct; compaction retrain + renumber correctness (merged segment answers
  correctly; delete `.mvpq` → reopen → `.mvec` fallback still correct); **V5/V6-unconfigured paths
  byte-identical** (fixed query, identical `search_multivector`/`search_rerank` top-k generation/docid/score vs
  an index built without multivector-PQ).
- **Borrowed-store UAF regression under ASan:** compact two segments that each carry a `.tann` built over
  `multivector_pq`; the merged segment's `search_multivector` must not UAF (refresh-before-rebuild ordering) —
  mirrors `test_v6_compaction`.
...
- **ASan/UBSan** clean on the new paths (codebook/codes/counts allocations, load validation, ADC-MaxSim,
  token-HNSW-over-PQ, compaction rebuild) — `-fsanitize=address,undefined` via `CC='g++ -fsanitize=...'`,
...
1. `ANT_multivector_pq_store` + `.mvpq` save + forgiving load + ADC-MaxSim + `ANT_multivector_pq_source`
   (unit-tested; codec reused from Phase 1).
...
4. Token-HNSW built over `ANT_multivector_pq_source` + **replace** posture (ADC-MaxSim) wired into
   `search_multivector`; V5/V6-unconfigured byte-identical regression lock.
5. **Rerank** posture (PQ candidate shortlist → exact float MaxSim rescore via resident `.mvec`).
6. Compaction retrain + renumber `.mvpq` with the borrowed-store refresh-before-`.tann`-rebuild ordering.
7. Recall sanity at default `m` + ASan/UBSan sweep (incl. `detect_leaks=1` on compaction).

Phase 2 is a shippable milestone. Dropping the resident float pool for the full replace-mode memory win, a
true int8 rerank tier, OPQ, global codebooks, `k != 256`, and bindings are deferred.
