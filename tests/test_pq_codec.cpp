...
static void test_m_must_divide(void)
{
	...
	CHECK(ANT_pq_codec::train(v, 6, 4, ANT_pq_codec::K, 1, cb) == 1);	// 4 does not divide 6
}
...
static void test_determinism_and_recall(void)
{
	...
	CHECK(ANT_pq_codec::train(data, dim, m, ANT_pq_codec::K, n, cb1) == 0);
	CHECK(ANT_pq_codec::train(data, dim, m, ANT_pq_codec::K, n, cb2) == 0);
	CHECK(memcmp(cb1, cb2, cbsize*sizeof(float)) == 0);		// deterministic

	unsigned char codes[4];
	double table[4*256];
	float q[16]; for (i=0;i<dim;i++) q[i]=(float)(rand()%200-100)/100.0f;
	ANT_pq_codec::adc_table(q, dim, m, ANT_pq_codec::K, cb1, ANT_pq_codec::METRIC_DOT, table);
	double maxerr = 0;
	for (long long d = 0; d < n; d++)
		{
		ANT_pq_codec::encode(data+d*dim, dim, m, ANT_pq_codec::K, cb1, codes);
		float recon[16]; ANT_pq_codec::reconstruct(codes, dim, m, ANT_pq_codec::K, cb1, recon);
		double adc = ANT_pq_codec::adc_score(codes, m, ANT_pq_codec::K, table);
...
static void test_degenerate_subspace(void)
{
	...
	CHECK(ANT_pq_codec::train(data, dim, m, ANT_pq_codec::K, n, cb) == 0);
	unsigned char codes[2]; float recon[4];
	ANT_pq_codec::encode(data, dim, m, ANT_pq_codec::K, cb, codes);
	ANT_pq_codec::reconstruct(codes, dim, m, ANT_pq_codec::K, cb, recon);
