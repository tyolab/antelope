...
class check {
	function get_extra_classes($ref = FALSE) {
		...
		if (! is_array ( $extra )) {
			...
			foreach ( $_original_classes as $class )
				unset ( $df [$class] );
			$extra = array_keys ( $df );
		}
		return $extra;
		...
		if (! is_array ( $extra )) {
			if (GETSET) {
				...
				foreach ( check::get_extra_functions ( false, 1 ) as $global ) {
					if (ereg ( '^(.*)_[sg]et$', $global, $match ))
						$_extra [$match [1]] = 1;
				}
				$extra = array_keys ( $_extra );
			} else {
				if ($ref === FALSE)
					$ref = $_original_globals;
				if (! is_array ( $extra )) {
					$df = array_flip ( array_keys ( $GLOBALS ) );
					foreach ( $_original_globals as $func )
						unset ( $df [$func] );
						// MASK xxxx_LOADED__ variables
					foreach ( array_keys ( $df ) as $func )
						if (ereg ( '_LOADED__$', $func ))
							unset ( $df [$func] );
					$extra = array_keys ( $df );
				}
			}
	...
	function classname($string, $object) {
		...
		if (strtolower ( $string ) != strtolower ( $classname = get_class ( $object ) ))
			return check::fail ( "Object: \$object is of class %s not class %s", $classname, $string );
		return TRUE;
	}
	function classmethods($classname, $methods) {
		...
		foreach ( $methods as $method ) {
			if (! isset ( $classmethods [$method] ))
				...
				unset ( $classmethods [$method] );
		}
		$extra = array_keys ( $classmethods );
		if ($missing)
			$message [] = "does not have these methods:\n  " . join ( ",", $missing );
		if ($message) {
			return check::fail ( "Class %s %s\nFull class list:\n  %s\n", $classname, join ( "\nbut ", $message ), join ( "\n  ", get_class_methods ( $classname ) ) );
		}
		if ($extra)
			$message [] = "Class " . $classname . " has these extra methods:\n  " . join ( ",", $extra );
		if ($message)
	...
	function is_a($a, $b) {
		if (is_object ( $a ))
			$a = strtolower ( get_class ( $a ) );
		if (is_object ( $b ))
			$a = strtolower ( get_class ( $b ) );
		$parents = array ();
		$c = $a;
		while ( $c != $b && $c ) {
			$parents [] = $c;
		...
		if ($c != $b)
			return check::fail ( "Class $a does not inherit from class $b\nHierachy:\n  %s\n", join ( "\n  ", $parents ) );
		return TRUE;
	}
	function classparent($a, $b) {
		...
		if ($parent != $b)
			return check::fail ( "Class $a parent not actually $b but $parent" );
		return TRUE;
	}
	function classes($classes) {
		...
		if ($missing)
			$message [] = sprintf ( "Classes missing: %s", join ( ",", $missing ) );
		if ($message)
			return check::fail ( join ( "\n  ", $message ) );
		if ($extra)
			$message [] = sprintf ( "These extra classes are defined: %s", join ( ",", array_keys ( $extra ) ) );
		if ($message)
			return check::warn ( join ( "\n  ", $message ) );
		return TRUE;
	}
	function functions($functions) {
		...
		if ($missing)
			$message [] = sprintf ( "Functions missing: %s", join ( ",", $missing ) );
		if ($message)
			return check::fail ( join ( "\n  ", $message ) );
		if ($extra)
			$message [] = sprintf ( "These extra functions are defined: %s", join ( ",", array_keys ( $extra ) ) );
		if ($message)
			return check::warn ( join ( "\n  ", $message ) );
		return TRUE;
	}
